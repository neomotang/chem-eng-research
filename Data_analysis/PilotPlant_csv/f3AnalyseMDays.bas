Attribute VB_Name = "f3AnalyseMDays"

Sub AnalyseMDays() 'analyse data from multiple days

'compiled August 2024

'This sub processes data from multiple days, determining the average values at specified times from the .csv files. For each day, these 3 .csv files with time stamped data are of interest:
...
Sheet1.Range("N17").Value = DataCount - 1 'total number of experimental runs, displayed on spreadsheet with tracking counter in cell M17
...
Else
    ...
    Do While DataCount > 1 'DoLoop#1
        
        folderDate = Format(Sheet1.Cells(fCounter, 1).Value, "yyyy-mm-dd") 'for the current experimental run entry, copies its date and formats it to match the folder name format
        DataFilePath = Sheet1.Range("F8").Value & folderDate & "\CSV\" 'the .csv files of interest are in the "CSV" subfolder
        
        Sheet1.Range("M17").Value = fCounter - 20 & "  of" 'progress counter displayed on spreadsheet
        
        'Open the PrTemp.csv file, paste it as a new tab at the end of the spreadsheet, and then paste the contents to start in column C of the "Worksheet" tab (Sheet2)
        ...
        'Open the PrDp.csv file, paste it as a new tab at the end of the spreadsheet, and then paste the contents to start in column AC of the "Worksheet" tab
        ...
        Do While RCounter <= RowCount 'DoLoop#2 - Determine the approximate start time in the .csv files from the difference between entries in the time column (C) and the user input experimental start time on the "Home" tab (Sheet1, column B)
            ...
            Else
                ...
                Do While RCounter <= RowCount 'DoLoop#3 - Determine the approximate end time in the .csv files from the difference between entries in the time column (C) and the user input experimental end time on the "Home" tab (Sheet1, column C)
        ...
        Do While RCounter <= dpRowCount 'DoLoop#4 - Determine the approximate start time in the .csv file from the difference between entries in the time column (AC) and the user input experimental start time on the "Home" tab (Sheet1, column B)
            ...
            Else
                ...
                Do While RCounter <= dpRowCount 'DoLoop#5 - Determine the approximate end time in the .csv file from the difference between entries in the time column (AC) and the user input experimental end time on the "Home" tab (Sheet1, column C)
        ...
        Sheets(Sheets.Count).Delete 'delete "PrFlow" tab
        Sheets(Sheets.Count).Delete 'delete "PrTemp" tab
        Sheet2.Activate
        Sheet2.Cells.Select
        Selection.Value = "" 'clear "Worksheet" tab
        
        fCounter = fCounter + 1 'move on to next row on "Home" tab
        DataCount = DataCount - 1 'reduce the number of experimental run entries to be analysed by 1
    Loop
