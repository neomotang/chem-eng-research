Attribute VB_Name = "f2GetDayFolder"

Sub GetDayFolderPathNFileCount()
    
'compiled August 2024

'This sub gets the folder path containing a single day's data in its own subfolder, and also counts the number of .csv files in the folder to be analysed
'The folder path, number of subfolders and number of files are saved to cells F8 - F10 on the "Home" tab (Sheet1)
    
Dim fldr As FileDialog

Set fldr = Application.FileDialog(msoFileDialogFolderPicker) 'choose folder containing single day's data
fldr.Title = "Select the folder containing your 1 day's .csv data"
...
If fldr.Show <> -1 Then
    ...
    Else:
        ...
        folderCount = CountFolders(fldr.SelectedItems(1))
