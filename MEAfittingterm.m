function out = MEAfittingterm(expdata)
...
%%lb = bounds(1);
%%ub = bounds(2);
k0 = [1 1]; %k0(1) = initial guess for amine; k0(2) = water;

% Wrapper function for minimization function
PredictionErrork = @(k) PredictionError(Xexp, ConversionPrediction(tspan,k));

% Minimization
%[out.kOpt, out.predErrOpt, out.exitflag, out.output] = %fminsearch(PredictionErrork, k0);
options = optimset('largescale', 'off', 'display', 'off');
%[out.kOpt, out.predErrOpt, out.exitflag, out.output] = %fmincon(PredictionErrork,k0,[],[],[],[],zeros(1,3),20*ones(1,3)[],options);
[out.kOpt, out.predErrOpt, out.exitflag, out.output] = fminsearchbnd(PredictionErrork, k0, zeros(1,2),inf*ones(1,2),options);
...
dX = V*((((R+1)*Fm*t-(R+2)*X*Na0)/(k(1)*(R+1)*V)-k(2)*55)*((R+1)*(1-
X)*Fm*t-(R+2)*(1-X)*X*Na0)/((R+1)*V^2));
